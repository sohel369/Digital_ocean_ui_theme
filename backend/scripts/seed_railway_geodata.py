"""
Seed US Geographic Data to Railway PostgreSQL Database
This script can be run on Railway to populate all US states data.
Run from Railway backend terminal: python backend/scripts/seed_railway_geodata.py
"""
import sys
import os
...
def seed_us_geodata():
    """Seed all 50 US states + DC geographic data"""
    db = SessionLocal()
    print(f"🔗 Connected to: {engine.url.database}")
    
    # Complete US States data with 2026 projections
    us_states_data = [
    ...
    try:
        ...
        print(f"📊 Total states to process: {len(us_states_data)}\n")
        ...
        for name, code, fips, pop, dens_mi, rank, pct, area, radius_count, mult in us_states_data:
            ...
            if existing:
                ...
                print(f"  ✅ Updated: {name} ({code}) - Pop: {pop:,}")
                updated += 1
            else:
                ...
                db.add(new_geo)
                print(f"  ➕ Added: {name} ({code}) - Pop: {pop:,}")
                added += 1
        
        db.commit()
        
        print("\n" + "="*60)
        print("🎉 US Geodata seeding complete!")
        print(f"   ➕ Added: {added} states")
        print(f"   ✅ Updated: {updated} states")
        print(f"   📊 Total: {added + updated} states")
        print("="*60)
        
        # Verify count
        total_us_states = db.query(models.GeoData).filter(
            models.GeoData.country_code == "US"
        ).count()
        print(f"\n✅ Verification: {total_us_states} US states now in database")
        
    except Exception as e:
        print(f"\n❌ Error seeding geodata: {e}")
        import traceback
        traceback.print_exc()
        db.rollback()
    finally:
...
if __name__ == "__main__":
    print("="*60)
    print("🗺️  Railway Geodata Seeder - US States")
    print("="*60)
    seed_us_geodata()
    print("\n✅ Script completed!")
